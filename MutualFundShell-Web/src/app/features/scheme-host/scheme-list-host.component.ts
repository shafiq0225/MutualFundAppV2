...
@Component({
  selector: 'shell-scheme-list-host',
  standalone: true,
  ...
  template: `
    @if (ready()) {
      ...
      <div class="remote-error">
        ...
        Scheme Management is unavailable right now. Confirm MutualFundScheme-Web
        is running on port 4205 and try again.
      </div>
    } @else {
      <div class="remote-loading">
        <i class="fas fa-circle-notch fa-spin"></i> Loading Scheme Management…
      </div>
    }
  `,
  styles: [`
...
export class SchemeListHostComponent implements OnInit {
